...
public class HandlePoisonSpecial
{
    public static WorldPoint findNearbySafeTile(Client client, AlchemicalHelperPlugin plugin, AlchemicalHelperConfig config)
    {
        NPC hydra = plugin.getHydra().getNpc();
        if (hydra == null)
        {
            return null;
        }

        //Build the dangerous tile set before searching for the safe tile, consisting of flame, poison splash tiles, and the Hydra's occupied tiles
        Set<WorldPoint> dangerousTiles = plugin.getDangerousTiles();
        Set<GraphicsObject> flameObjects = plugin.getFlameObjects();
        for (GraphicsObject flame : flameObjects)
        {
            dangerousTiles.add(WorldPoint.fromLocal(client, flame.getLocation()));
        }
        //Calculate the splash area of the poisonProjectile and add it to the set of dangerous tiles
        for (Map.Entry<LocalPoint, Projectile> poison : plugin.getPoisonProjectiles().entrySet())
        {
        ...
        //Hydra's occupied tiles, add it to the dangerous tile set and use the area to calculate for melee distance later
        WorldArea hydraArea = hydra.getWorldArea();
        dangerousTiles.addAll(hydraArea.toWorldPointList());
        ...
        while (!queue.isEmpty())
        {
            ...
            if (y < 127 && canMoveNorth(flags[x][y]) && canMoveTo(flags[x][y + 1]) && !visitedTiles.contains(tiles[x][y + 1]))
            {
                ...
                if (isTileSafe(northNeighbor, dangerousTiles))
                {
                     ...
                     */
                    if (VarUtilities.getPlayerAttackStyle() == 0 && config.favorMeleeDistanceDuringPoison() && flameObjects.isEmpty())
                    {
                        if (hydraArea.isInMeleeDistance(northNeighbor.getWorldLocation()))
                        {
            ...
            if (x < 127 && canMoveEast(flags[x][y]) && canMoveTo(flags[x + 1][y]) && !visitedTiles.contains(tiles[x + 1][y]))
            {
                ...
                if (isTileSafe(eastNeighbor, dangerousTiles))
                {
                    ...
                    if (VarUtilities.getPlayerAttackStyle() == 0 && config.favorMeleeDistanceDuringPoison() && flameObjects.isEmpty())
                    {
                        if (hydraArea.isInMeleeDistance(eastNeighbor.getWorldLocation()))
                        {
            ...
            if (y > 0 && canMoveSouth(flags[x][y]) && canMoveTo(flags[x][y - 1]) && !visitedTiles.contains(tiles[x][y - 1]))
            {
                ...
                if (isTileSafe(southNeighbor, dangerousTiles))
                {
                    ...
                    if (VarUtilities.getPlayerAttackStyle() == 0 && config.favorMeleeDistanceDuringPoison() && flameObjects.isEmpty())
                    {
                        if (hydraArea.isInMeleeDistance(southNeighbor.getWorldLocation()))
                        {
            ...
            if (x > 0 && canMoveWest(flags[x][y]) && canMoveTo(flags[x - 1][y]) && !visitedTiles.contains(tiles[x - 1][y]))
            {
                ...
                if (isTileSafe(westNeighbor, dangerousTiles))
                {
                    ...
                    if (VarUtilities.getPlayerAttackStyle() == 0 && config.favorMeleeDistanceDuringPoison() && flameObjects.isEmpty())
                    {
                        if (hydraArea.isInMeleeDistance(westNeighbor.getWorldLocation()))
                        {
    ...
    private static boolean isTileSafe(Tile tile, Set<WorldPoint> dangerousTiles)
    {
        ...
        if (dangerousTiles.contains(tileWorldPoint))
        {
            return false;
        }
        ...
        for (GameObject gameObject : gameObjects)
        {
            ...
            //GameObject 26209 is the invisible wall that pops up during the flame wall special.
            if (gameObjectId == ObjectID.CHEMICAL_VENT_BLUE || gameObjectId == ObjectID.CHEMICAL_VENT_GREEN ||
                    gameObjectId == ObjectID.CHEMICAL_VENT_RED || gameObjectId == 26209)
            {
                return false;
            }
        }
